...
5. Во время правок dev-сервер Turbopack залип в цикле пересборки на каждый запрос (в логах `CJS module can't be async`, потом `ChunkLoadError`) — страница открывалась, но каждый запрос стал занимать 3+ секунды. Помогла очистка `.next` и перезапуск сервера.
6. Попутно нашла косяк в тексте: было «994 результатов» — некорректное склонение для любых чисел, кроме 2–4. Заменила на «Найдено: N», чтобы не тащить логику склонений ради одной строки.
